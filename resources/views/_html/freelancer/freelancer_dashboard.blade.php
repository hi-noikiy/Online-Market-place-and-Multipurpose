...
@php
    ...
    $proposal=App\FreelancerProjectProposal::where('user_id',$user->id)->get();
    $project_won=App\FreelancerProjectProposal::where('user_id',$user->id)->where('status',1)->count();
    $gig=App\Gig::where('freelancer_id',$freelancer->id)->get();
    $proposal_got=0;
    if($gig){
      foreach($gig as $gig){
        $c=App\FreelancerProjectProposal::where('gig_id',$gig->id)->where('status',0)->count();
...
@endphp
  <div class="w3-container" id="cop">
      ...
      {{-- <div class="w3-third">
        <div class="w3-container w3-teal w3-padding-16" style="border-radius: 5px;">
          <div class="w3-left"><i class="fas fa-archive w3-xxxlarge"></i></div>
          <div class="w3-right">
            <h3>1-12-19</h3>
          </div>
          <div class="w3-clear"></div>
          <h4>Next Submission</h4>
        </div>
      </div> --}}
  ...
  @if($active_task)
   @foreach($active_task as $task)
     @php
         $job=App\Job::find($task->project_id);
          $pro_request_current=App\FreelancerProjectProposal::where('project_id',$job->id)->where('status',1)->first();
...
<div class="w3-col m3" style="margin-left: 16px;">
  <div class="w3-panel w3-teal w3-display-container" style="border-radius: 5px;">
